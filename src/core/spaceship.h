...
struct SegmentData{ 
	...
	//bit 6: reserved
	//bit 7: reserved
	cu8 Radius1; //the in-game radius of one side of this segment.
	cu8 Radius2; //the in-game radius of the other side of this segment. Only used for cones.
	cu8 Height; //the in-game height of this segment. Not used for base segments
	cu8 GfxWidth; //the width of the texture in pixels.
	cu8 GfxHeight; //the height of the texture in pixels.
	cu8 *GfxData; //pointer to the start of this segments texture.
};

//This is the data for one part. A rocket is built up from many of these pre-defined parts
struct PartData{
	cu8 NumSegments; //the number of segments needed to draw this part.
	const struct SegmentData *SegmentsPtr; //a pointer to an array of NumSegments SegmentData structs.
...
struct ColumnData{
	u8 NumParts; //the number of parts that make up this column.
	struct PartData *PartsPtr; //a pointer to an array of NumParts PartData structs.
	u32 Length; //The total length of this column in in-game units, from tip to tail.
	s32 XPos; //The X position of this column relative to the center column in in-game units, X is towards the camera when Spin and Pitch = 0.
	s32 ZPos; //The Z position of this column relative to the center column in in-game units, Z is towars the top of the screen when Spin and Pitch = 0.
	s32 YPos; //The Y position of the top of this column relative to the top of the center column in in-game units.
};
...
struct ShipData{
	u32 NumColumns; //the number of columns that make up this ship.
	struct ColumnData *ColumnsPtr; //a pointer to an array of NumColumns ColumnData structs.
